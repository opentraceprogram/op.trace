...
#
# Hacer esta comprobacion UNA SOLA VEZ al arrancar el programa (como
# se hacia antes) no siempre basta: en servidores como Render, el
# programa se ejecuta con gunicorn, que puede repartir las peticiones
# entre varios procesos o hilos segun como este configurado, y no hay
# garantia de que sea exactamente el mismo hilo donde se hizo esta
# comprobacion al arrancar. Por eso, ADEMAS de comprobarlo aqui al
# arrancar, volvemos a comprobarlo justo antes de crear cada conexion
# nueva con Telegram (llamando a asegurar_event_loop() dentro de
# login()), que es el momento en el que de verdad hace falta.
try:
...
def asegurar_event_loop():
    """
    Se asegura de que el hilo que esta atendiendo la peticion ACTUAL
    tenga un bucle de eventos de asyncio valido, y lo crea si no lo
    tiene. Hay que llamar a esto justo antes de crear un
    TelegramClient nuevo (que es el momento en el que Telethon lo
    necesita).
    """
    try:
        bucle = asyncio.get_event_loop()
        if bucle.is_closed():
            raise RuntimeError("El bucle de eventos anterior esta cerrado.")
    except RuntimeError:
...
# CUANDO SE EJECUTA EN TU PROPIO ORDENADOR. Gracias a este archivo, la
# proxima vez que abras la pagina no hara falta volver a meter el
# codigo de verificacion.
SESSIONS_DIR = os.path.join(os.path.dirname(__file__), "sessions")
os.makedirs(SESSIONS_DIR, exist_ok=True)

# Render pone automaticamente esta variable de entorno en todos sus
# servicios. La usamos para detectar si la app se esta ejecutando ahi.
#
# Por que hace falta distinguirlo: el archivo de sesion (.session) es
# en realidad una base de datos SQLite, y SQLite necesita que el
# sistema de archivos soporte bien los "bloqueos de archivo" del
# sistema operativo. El disco efimero de Render (y el de otras
# plataformas similares) no siempre lo soporta correctamente, lo que
# puede hacer que SQLite falle con "database is locked" incluso en el
# primerisimo intento, sin que haya ningun otro proceso tocando el
# archivo a la vez.
#
# La solucion es no usar ningun archivo en Render: en vez de eso,
# usamos una "StringSession" de Telethon, que guarda la sesion como un
# simple texto en la memoria del programa, sin tocar el disco para
# nada. El unico "coste" es que, como el disco de Render ya era
# efimero de por si (se borra en cada reinicio/despliegue), la sesion
# tampoco sobrevive a un reinicio del servicio, exactamente igual que
# pasaba antes con el archivo.
EN_RENDER = os.environ.get("RENDER", "").strip().lower() == "true"
...
def buscar_cancionvip(term):
    try:
        r = requests.post(
            "https://cancion.vip/api/search",
            json={
                "term": term,
                "fields": [
                    "uuid",
...
def login():
    ...
    try:
        with bloqueo:
            ...
            # Nos aseguramos de que el hilo que atiende ESTA peticion
            # tenga un bucle de eventos valido antes de crear el
            # cliente de Telegram (ver la explicacion junto a
            # asegurar_event_loop, mas arriba).
            asegurar_event_loop()

            if EN_RENDER:
                # En Render usamos una sesion en memoria (ver la
                # explicacion junto a EN_RENDER mas arriba), que no
                # toca el disco para nada.
...
def buscar_cancion():
    ...
    resultados_cancion = buscar_cancionvip(mensaje)
    ...
    try:
        ...
        if resultados_cancion:
            respuesta["cancionvip"] = resultados_cancion
